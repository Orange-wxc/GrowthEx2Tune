...
class TD3(Algorithm):
    ...
    def critic_learn(self, obs, action, reward, next_obs, terminal):
        print("******************************************")
        print("**                                      **")
        print("**       this is func critic learn      **")
        print("**                                      **")
        print("******************************************")
        print("action = ", action)
    ...
    def cal_td_error(self, obs, action, reward, next_obs, terminal):
        print("******************************************")
        print("**                                      **")
        print("**       this is func cal_td_error      **")
        print("**                                      **")
        print("******************************************")
       # print("action = ", action)
       # print("action shape = ", len(action)) 
       # action_new = np.array(action).astype('float32').reshape(1, len(action[0]))
       ...
       # action = action_new
        ...
        current_Q1, current_Q2 = self.model.value(obs, action)
        # print("current Q1 = ", current_Q1.item())
        # td_error = abs(current_Q1 - target_Q) + abs(current_Q2 - target_Q)
        # td_error = fluid.layers.abs(fluid.layers.sum([current_Q1, -target_Q]))
        td_error = fluid.layers.elementwise_add(
            fluid.layers.abs(fluid.layers.elementwise_add(current_Q1, -target_Q)),
            fluid.layers.abs(fluid.layers.elementwise_add(current_Q2, -target_Q))
        )
        print("******************************************")
        print("**                                      **")
